...
#ifdef CONFIG_AFC
static int hostapd_afc_parse_sock(struct hostapd_config *conf, char *pos)
{
	struct sockaddr_in6 *sin6 = &conf->afc.sock;
	char addr[INET6_ADDRSTRLEN] = {};
	int port;

	if (sscanf(pos, "[%39[^]]]:%d", addr, &port) != 2)
		return -EINVAL;

	memset(sin6, 0, sizeof(*sin6));
	sin6->sin6_family = AF_INET6;
	sin6->sin6_port = htons(port);
	return inet_pton(AF_INET6, addr, &sin6->sin6_addr) == 1 ? 0 : -EINVAL;
}
			       ...
			       const char *buf, char *pos, int line)
{
	if (os_strcmp(buf, "interface") == 0) {
		...
		if (hostapd_afc_parse_sock(conf, pos))
			return 1;
