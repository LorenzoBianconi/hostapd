...
static bool nl80211_link_valid(struct i802_bss *bss, s8 link_id)
{
	...
	for (i = 0; i < bss->n_links; i++) {
		wpa_printf(MSG_DEBUG, "nl80211: i=%u, link_Id=%u",
			   i, bss->links[i].link_id);
...
static int i802_set_sta_vlan(struct i802_bss *bss, const u8 *addr,
			     const char *ifname, int vlan_id, s8 link_id)
{
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg;
	int ret;
	...
	if (!(msg = nl80211_bss_msg(bss, 0, NL80211_CMD_SET_STATION)) ||
	    ...
	    (link_id >= 0 && nla_put_u8(msg, NL80211_ATTR_MLO_LINK_ID,
					link_id)) ||
