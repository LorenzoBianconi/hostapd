...
{
	int sockfd, err, request_timeout = HOSTAPD_AFC_RETRY_TIMEOUT;
	struct hostapd_config *iconf = iface->conf;
	...
	err = connect(sockfd, (struct sockaddr *)&iconf->afc.sock,
		      sizeof(iconf->afc.sock));
	if (err < 0) {
	...
	err = hostapd_afc_parse_reply(iface, buf);
	if (err > 0) {
		request_timeout = err;
		err = 0;
	}
