				      ...
				      int reassoc)
{
	...
	wpa_printf(MSG_INFO, "MLD: link: link_id=%u, peer=" MACSTR,
		   hapd->conf->mld_link_id, MAC2STR(link->peer_addr));
					  ...
					  int reassoc)
{
	 ...
	 */
	sta->mld_info.links[hapd->conf->mld_link_id].rsne =
		hostapd_wpa_ie(hapd, WLAN_EID_RSN);
	sta->mld_info.links[hapd->conf->mld_link_id].rsnxe =
	...
	for (i = 0; i < MAX_NUM_MLD_LINKS; i++) {
		...
		for (j = 0; j < hapd->iface->interfaces->count; j++) {
			...
			if (iface->bss[0]->conf->mld_ap &&
			    ...
			    i == iface->bss[0]->conf->mld_link_id)
				break;
		}
			 ...
			 struct hostapd_data **assoc_hapd)
{
	...
	*assoc_hapd = hapd;
	...
	/* Find the hapd with the corresponding link ID */
	for (i = 0; i < hapd->iface->interfaces->count; i++) {
		...
		if (other_hapd->conf->mld_ap &&
		    hapd->conf->mld_id == other_hapd->conf->mld_id &&
		    sta->mld_assoc_link_id == other_hapd->conf->mld_link_id)
			break;
	}

	if (!other_hapd || i == hapd->iface->interfaces->count) {
		wpa_printf(MSG_DEBUG,
			   "MLD: no link match for link_id=%u",
			   sta->mld_assoc_link_id);
		return sta;
...
#endif /* CONFIG_MESH */
#ifdef CONFIG_IEEE80211BE
	    !(hapd->conf->mld_ap && !os_memcmp(hapd->mld_addr, mgmt->bssid,
					       ETH_ALEN)) &&
#endif /* CONFIG_IEEE80211BE */
					   ...
					   int ok)
{
	...
	/*
	 * TODOs:
	 * - 802.1X port enablement is not needed as done on the station doing
	 *     the connection.
	 * - Not handling accouting
...
#ifdef CONFIG_IEEE80211BE
	if (hapd->conf->mld_ap &&
	    hapd->conf->mld_link_id != sta->mld_assoc_link_id) {
		wpa_printf(MSG_INFO,
