...
SM_STATE(WPA_PTK, PTKSTART)
{
	...
	if (sm->mld_assoc_link_id >= 0) {
		wpa_printf(MSG_DEBUG,
			   "RSN: MLD: Adding MAC Address KDE: kde_len=%zu",
			   kde_len);
					...
					struct wpa_eapol_ie_parse *kde)
{
	...
	if (!kde->mac_addr ||
		...
		wpa_printf(MSG_DEBUG,
			   "WPA_AUTH: MLD: invalid MLD address");
		return -1;
	}
	...
	for (i = 0; i < MAX_NUM_MLD_LINKS; i++) {
		 ...
		 * address
		 */
		if (kde->mlo_link_len[i] != 7) {
			wpa_printf(MSG_DEBUG,
				   "WPA_AUTH: MLD: invalid MLO link KDE len=%zu",
				   kde->mlo_link_len[i]);
			return -1;
		}
		...
		if (!sm->mld_links[i].valid || i == sm->mld_assoc_link_id) {
			wpa_printf(MSG_DEBUG,
				   "WPA_AUTH: MLD: invalid link id=%u", i);
			return -1;
		}

		if (os_memcmp(sm->mld_links[i].peer_addr,
			      kde->mlo_link[i] + 1,
			      ETH_ALEN)) {
			wpa_printf(MSG_DEBUG,
				   "WPA_AUTH: MLD: invalid MAC address=" MACSTR,
	...
	if (n_links != sm->n_mld_affiliated_links) {
		wpa_printf(MSG_DEBUG,
			   "WPA_AUTH: MLD: expecting %u mld links in m2, but got %u",
